...
class ExpandPaddingStrategy extends BaseStrategy {
  constructor() {
    super('variant_padding', 'Expanded Padding');
  }
  ...
  async run(request, context) {
    ...
    const output = await context.imagePatcher.allocateVariantPath(nodeId, 'pad20');
    const manualPadding = Number(request.manualParams && request.manualParams.expandPadding);
    const expandPadding = Number.isFinite(manualPadding)
      ? Math.max(1, Math.round(manualPadding))
      : 20;
    ...
    await context.browserSession.execute(request.htmlPath, async ({ page }) => {
      ...
      try {
        const capture = await page.evaluate((targetSelector, options) => {
            ...
            for (const control of controls) {
              ...
              if ('value' in control) {
                try {
                  ...
                  // ignore runtime control write failures
                }
              }
          ...
          clone.style.left = `${options.expandPadding}px`;
          clone.style.top = `${options.expandPadding}px`;
          ...
          return {
            clip: {
              x: 0,
              y: 0,
              width: Math.max(1, Math.ceil(rect.width + options.expandPadding * 2)),
              height: Math.max(1, Math.ceil(rect.height + options.expandPadding * 2)),
            },
            metadata: {
              expandPadding: options.expandPadding,
            },
          };
        }, selector, {
          expandPadding,
        ...
        if (!capture || !capture.clip) {
          throw new Error(`ExpandPaddingStrategy: target not found for ${nodeId}`);
        }

        const clip = this.normalizeClip(capture.clip);
        if (!clip) {
          throw new Error(`ExpandPaddingStrategy: invalid clip for ${nodeId}`);
        }
    ...
    return this.createVariant({
      ...
      description: 'Expand capture bounds to reduce clipping',
      metadata: {
        strategy: 'EXPAND_PADDING',
        contentOffsetX: -expandPadding,
        contentOffsetY: -expandPadding,
        expandPadding,
      },
    });
  }
...
module.exports = ExpandPaddingStrategy;
